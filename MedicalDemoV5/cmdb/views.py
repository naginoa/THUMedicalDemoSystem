...
def get_backbone(request):
    imgbackbone = request.FILES.get('file10')
    print('-----------------------------', imgbackbone)
    img = utils.socket_client(imgbackbone, 12132)
    bak_dict = {}
    bak_dict["desc"] = img

    return JsonResponse(bak_dict)

@require_http_methods(['GET','POST'])
def get_teeth(request):
    imgteeth = request.FILES.get('file5')
    print('-----------------------------', imgteeth)
    img = utils.socket_client(imgteeth, 12131)
    img_dict={}
    img_dict["desc"] = img
